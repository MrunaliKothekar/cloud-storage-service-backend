...
import { canView, getResourcePermission } from "../services/permission.service.js";
...
export const getRootContents = async (req: Request, res: Response) => {
  try {
    ...
    const [foldersResult, filesResult] = await Promise.all([
      pool.query(
        `
        SELECT id, name, owner_id, parent_id, is_deleted, created_at, updated_at
        FROM folders
        WHERE owner_id = $1
      ...
      pool.query(
        `
        SELECT id, name, mime_type, size_bytes, folder_id, owner_id,
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to get root contents",
    });
  }
};
...
export const getFolder = async (req: Request, res: Response) => {
  try {
    ...
    const access = await canView(userId, "folder", id);
    if (!access) {
    ...
    const [foldersResult, filesResult, pathResult] = await Promise.all([
      pool.query(
        `
...
export const updateFolder = async (req: Request, res: Response) => {
  try {
    ...
    const role = await getResourcePermission(userId, "folder", id);

    if (!role || !["owner", "editor"].includes(role)) {
      return res.status(403).json({ success: false, message: "You do not have permission to edit this folder" });
    }

    const newName = name !== undefined ? String(name).trim() : currentFolder.name;
    const newParentId = parentId !== undefined ? parentId : currentFolder.parent_id;
    const isOwner = currentFolder.owner_id === userId;

    if (!newName || newName.length > 255) {
      return res.status(400).json({ success: false, message: "Folder name must be between 1 and 255 characters" });
    }

    if (!isOwner && newParentId !== currentFolder.parent_id) {
      return res.status(403).json({
        success: false,
        message: "Editors can rename shared folders but cannot move them",
      });
    }
    ...
    if (newParentId !== null) {
      const parentResult = await pool.query(
        ...
        `,
        [newParentId, currentFolder.owner_id]
      );
      ...
      const descendantResult = await pool.query(
        ...
        `,
        [id, currentFolder.owner_id, newParentId]
      );

      if (descendantResult.rows.length > 0) {
        return res.status(400).json({ success: false, message: "Cannot move a folder inside one of its descendants" });
      }
    }
    ...
    const duplicateResult = await pool.query(
      ...
      WHERE owner_id = $1
        ...
        AND is_deleted = false
      `,
      [currentFolder.owner_id, newParentId, newName, id]
    );
    ...
    return res.status(200).json({ success: true, folder: updatedFolder });
  } catch (error) {
